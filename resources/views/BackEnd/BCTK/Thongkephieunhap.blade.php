...
@section('title', 'Thống kê nhập kho')

@section('content-backend')
@include('flash::message')  
<p class="h2"><img src="{{ asset('upload/icon/Pie Chart_30px.png') }}"> Thống kê nhập kho</p>
<div class="row" style="margin-top: 20px;">
    <form class="col-2" action="{{route('baocaotonkho.phieunhap')}}" method="GET" id="form_loaitk_tkbc">
        <select class="custom-select {{ $errors->has('khohang_id') ? ' is-invalid' : '' }}" name="loai" id="select_loaitk_tkbc">
          <option value="1" {{ ($a['loai'] == 1)? 'selected' : ''}}> Theo phiếu nhập</option>
          <option value="2" {{ ($a['loai'] == 2)? 'selected' : ''}}> Theo vật tư </option>
        </select>
    </form>
    <form class="col-8" action="{{route('baocaotonkho.phieunhap')}}" method="GET">
        <div id="search_user">
            <div class="form-row">
    ...
    <form class="col-2" action="{{route('baocaotonkho.exportPN')}}" method="POST" id="form_khohang_tonkho">
    	 @csrf
