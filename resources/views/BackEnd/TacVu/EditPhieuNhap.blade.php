...
@section('title', 'Sửa thông tin phiếu nhập kho')

@section('content-backend')
    <div id="register">
        <p class="h2" style="margin-bottom: 20px;"><img src="{{ asset('upload/icon/Add File_30px.png') }}"> Sửa thông tin phiếu nhập kho</p>
        @foreach($phieunhaps as $pn)
            <div class="">
                    ...
                    <div class="row col-md-12">
                          ...
                          <div class="col-md-9">
                              <table class="table table-bordered table-nhapxuat-details" style="margin-bottom: 30px;">
                                ...
                                <tbody>
                                  @foreach($phieunhapdetails as  $pnd)
                                    <tr>
                                        <th scope="row" style="vertical-align:middle; text-align: center;">{{ $loop->iteration }}</th>
                                        <td style="vertical-align:middle; text-align: center;">
                                          <span style="font-weight: bold;">{{ $pnd->tenvt }}</span>
                                        </td>
                                        <td style="vertical-align:middle; text-align: center;">
                                          <input type="hidden" class="dongia_nhapxuat" value="{{$pnd->dongia}}">
                                          <span style="font-weight: bold;" >{{ Helper::Tien($pnd->dongia) }}</span> VNĐ
                                        </td>
                                        <td style="vertical-align:middle; text-align: center;">
                                          <input type="number" name="soluong_session" class="form-control soluong_phieunhap" data-id="{{$pnd->id}}" value="{{ $pnd->soluong }}">
                                        </td>
                                        <td style="vertical-align:middle; text-align: center;">
                                          <span style="font-weight: bold;" class="amount_nhapxuat">0</span> VNĐ
                                        </td>
                                        <td style="vertical-align:middle; text-align: center;">
                                          <input type="number" class="form-control baohanh_phieunhap" data-id="{{$pnd->id}}" value="{{ $pnd->baohanh }}">
                                        </td>
                                        <td align="center"  style="vertical-align:middle;">
                                          <form action="{{route('chitietphieunhap.deleterow')}}" method="POST">
                                            {{ csrf_field() }}
                                            <input type="hidden" name="id" value="{{$pnd->id}}">
                                            <input type="hidden" name="phieunhap_id" value="{{$pn->id}}">
                  ...
                  <form method="POST" action="{{ route('quanlyphieunhap.update',$pn->id) }}" enctype="multipart/form-data">
                    @csrf
                    {{ method_field('PUT') }}
                  <div class="form-row" style=" margin-bottom: 10px;">
                    <div class="col-md-6">
                      <div class="form-group row">
                          ...
                          <div class="col-md-8">
                              <input  type="text" class="form-control {{ $errors->has('users_id') ? ' is-invalid' : '' }}" value="{{ $pn->ngtao }}" readonly="true">
                              <input  type="hidden" name="users_id" value="{{ $pn->users_id }}">
                      ...
                      <div class="form-group row">
                          ...
                          <div class="col-md-5">
                              <input type="text" class="form-control{{ $errors->has('ncc_id') ? ' is-invalid' : '' }}" placeholder="Nhà cung cấp" value="{{ $pn->ncc }}" readonly="true" id="ncc_name">
                              <input type="hidden" name="ncc_id" id="ncc_id_hidden" value="{{ $pn->ncc_id }}">
                      ...
                      <div class="form-group row">
                          ...
                          <div class="col-md-8">
                              <select class="custom-select {{ $errors->has('khohang_id') ? ' is-invalid' : '' }}" name="khohang_id">
                                ...
                                <option value="{{$kh->id}}" {{ ($pn->khohang_id == $kh->id)? 'selected' : '' }} >{{$kh->name}}</option>
                                @endforeach
                ...
                <div class="col-md-4">
                    <div class="form-group row">
                        ...
                        <div class="col-md-7">
                            <input type="date" class="form-control {{ $errors->has('ngaynhap') ? ' is-invalid' : '' }}" name="ngaynhap" value="{{ $pn->ngaynhap }}">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-7">
                            <input type="text" class="form-control {{ $errors->has('thanhtien') ? ' is-invalid' : '' }}"  value="{{$pn->thanhtien}}" readonly="true" id="tongtien_phieunhap">
              ...
              <div class="form-row">
                <div class="row col-md-12">
                      ...
                      <div class="col-md-9">
                          <textarea style="height: 150px; color: black !important; font-size: 15px !important;" class="form-control {{ $errors->has('mota') ? ' is-invalid' : '' }}" name="mota" placeholder="Ghi chú thông tin..." >{{$pn->mota}}</textarea>
                      </div>
                  </div>
              </div>
...
<form id="form_phieunhap_details" method="POST" action="{{ route('chitietphieunhap.store') }}">
  @csrf
  <input type="hidden" name="phieunhap_id" value="{{$pn->id}}">
  <input type="hidden" name="supplie_id" id="supplies_id_phieunhap">
  <input type="hidden" name="khohang_id" value="{{$pn->khohang_id}}">
...
<form id="form_editsoluong_phieunhap" method="POST" action="{{ route('chitietphieunhap.editsoluong') }}">
  @csrf
  <input type="hidden" name="phieunhap_id" value="{{$pn->id}}">
...
<form id="form_editbaohanh_phieunhap" method="POST" action="{{ route('chitietphieunhap.editbaohanh') }}">
  @csrf
  <input type="hidden" name="phieunhap_id" value="{{$pn->id}}">
