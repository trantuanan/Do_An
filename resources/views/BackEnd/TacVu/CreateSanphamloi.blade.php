...
@section('title', 'Thêm sản phẩm lỗi')

@section('content-backend')
    <div id="register">
        <p class="h2" style="margin-bottom: 20px;"><img src="{{ asset('upload/icon/Add File_30px.png') }}"> Tạo sản phẩm lỗi</p>
        <form method="POST" action="{{ route('quanlysanphamloi.store') }}" >
            @csrf
            <div class="">
                      ...
                      <div class="form-group row">
                          ...
                          <div class="col-md-5">
                              <select class="custom-select" name="tdsx_id" id="select_tdsx">
                                ...
                                @foreach($tdsxs as $td)
                                <option value="{{$td->id}}" data-id="{{$td->product_id}}" data-name="{{$td->tensp}}" data-max="{{$td->soluong}}">{{$td->matdsx}} - {{$td->tensp}}</option>
                                @endforeach
                    ...
                    <div class="form-group row">
                      ...
                      <div class="form-group row">
                        ...
                        <div class="col-md-5">
                          ...
                          <button type="button" class="btn btn-secondary" onclick="javascript:location.href='{{route('quanlysanphamloi.index')}}'">Hủy bỏ</button>
                        </div>
                      </div>
